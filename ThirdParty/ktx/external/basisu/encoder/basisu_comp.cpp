...
#ifndef BASISU_DISABLE_KTX2_ALIGNMENT_WORKAROUND
...
#undef PRINT_FLOAT_VALUE
	...
	void basis_compressor::get_dfd(uint8_vec &dfd, const basist::ktx2_header &header)
	{
		...
		if (header.m_supercompression_scheme != basist::KTX2_SS_NONE)
		{
			...
			// KTX 2.0.4 requires sized planes for BasisLZ DFDs. The pinned
			// upstream encoder still emits the deprecated zero form.
			plane_bits &= ~0xFFFF;
			plane_bits |= 8;
			if (m_any_source_image_has_alpha)
				plane_bits |= (8 << 8);
	...
	bool basis_compressor::create_ktx2_file()
	{
		...
		char writer_id[128];
#if defined(BASISU_STONER_KTX_WRITER_ID)
#ifdef _MSC_VER
		strcpy_s(writer_id, sizeof(writer_id), BASISU_STONER_KTX_WRITER_ID);
#else
		snprintf(writer_id, sizeof(writer_id), "%s", BASISU_STONER_KTX_WRITER_ID);
#endif
#else
#ifdef _MSC_VER
