...
- The backend may enable a native shader/pipeline object context independently
  of other still-deterministic `FVulkanDevice` objects. Only wrappers retaining
  a live native token may report `RealRuntime`; buffers and other symbolic
  objects are not relabeled.
...
- A real-runtime graphics pipeline retains its native pipeline, pipeline
  layout, descriptor set layouts, and compatible render pass until explicit
  invalidation or device shutdown.
...
- Graphics pipeline creation validates color and depth/stencil attachment
  formats against the selected device capabilities.

Negative-path requirements:

- Missing required shader stages, duplicate required stages, wrong shader stages, invalid or incompatible shader interface metadata, foreign-device dependencies, missing or invalidated layout, invalid vertex input, invalid topology, invalid fixed-function state, unsupported attachment formats, incompatible render target compatibility, configured failure limits, and post-shutdown creation return explicit failure.
- Failed creation does not populate successful process-local reuse entries.
- Native creation, wrapper allocation, tracking, and cache insertion publish
  atomically; every failure releases partial native state and returns no object.
...
- Real runtime pipeline creation is used when runtime support is available.
- A real-runtime compute pipeline retains its native pipeline, pipeline layout,
  and descriptor set layouts until explicit invalidation or device shutdown.
...
- Non-compute shader modules, missing shader modules, multiple compute shader modules where unsupported, incompatible shader interface metadata, foreign-device dependencies, invalidated dependencies, unsupported compute capability, configured failure limits, and post-shutdown creation return explicit failure.
- Failed creation does not populate successful process-local reuse entries.
- Native creation, wrapper allocation, tracking, and cache insertion publish
  atomically; every failure releases partial native state and returns no object.
...
- Reuse keys use collision-safe encoding and account for runtime mode, shader
  bytecode and identities, complete shader interface metadata, canonicalized
  pipeline layout declarations, render target compatibility, and all relevant
  pipeline state.
- Cache hits require the retained shader and layout dependencies to remain
  valid; a corrected request never reuses a stale pipeline.
...
- A valid graphics pipeline from the same device can be bound to a compatible
  recording graphics command buffer inside compatible render pass scope.
- A valid compute pipeline from the same device can be bound to a compatible
  recording compute-capable command buffer.
...
- Binding outside recording state, binding foreign-device pipelines, binding
  graphics pipelines without compatible render pass scope, binding compute
  pipelines to incompatible queues, binding the wrong pipeline kind, binding
  pipelines with invalidated dependencies, and binding incompatible render
  target/layout state return explicit failure.
...
- Collision-safe cache keys distinguish delimiter-ambiguous identities and
  differing shader interfaces; invalidated retained dependencies prevent hits.
- Command binding success and rejection for graphics and compute pipelines.
- Cross-device graphics and compute pipeline binding rejection.
- Native graphics and compute pipeline ownership through explicit invalidation
  and shutdown.
