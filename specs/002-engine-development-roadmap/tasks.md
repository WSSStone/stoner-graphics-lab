...
**Tests**: Documentation validation combines focused automated consistency scans with manual review of scope and evidence policy.
...
> Phases 1-8 below are historical delivery records and retain the ranges that
> were correct for Roadmaps 1.x-2.1. Phase 10 is the current 003-041 authority
> and supersedes those ranges for present-tense validation.
...
- [X] T012 [US1] Build or refresh the table of contents with links for all major roadmap sections and runtime Features 003 through 038 in `doc/roadmap.md`
- [X] T013 [US1] Build or refresh the phase overview table with runtime Features 003 through 038, layer, dependencies, complexity, critical path flag, and status in `doc/roadmap.md`
- [X] T014 [US1] Build or refresh the Mermaid dependency graph with runtime nodes P003 through P038 and all topological dependency edges in `doc/roadmap.md`
...
- [X] T016 [US1] Document the recommended solo-developer execution order from Feature 003 through Feature 038 in `doc/roadmap.md`
...
- [X] T025 [US2] Write or update additional Backend and advanced rendering phase detail sections 027 through 038 with required header fields, scope, deliverables, exclusions, and speckit prompts in `doc/roadmap.md`
...
- [X] T029 [US3] Ensure the phase overview table includes a status column for every runtime Feature 003 through 038 in `doc/roadmap.md`
...
- [X] T035 Validate that `doc/roadmap.md` satisfies SC-001 by containing runtime Features 003 through 038 across all 6 ownership areas
...
## Phase 8: Roadmap 2.1 Granularity and Ordering Amendment

**Purpose**: Correct Asset phase ordering, separate offline and runtime asset
delivery, and bound advanced rendering and native backend work to one
responsibility per Speckit cycle.

- [X] T051 Correct Feature 023/024 ordering so material/shader asset schemas precede glTF static model material subresources in `doc/roadmap.md`
- [X] T052 Split former Feature 025 into Feature 025 offline cooker/manifests and Feature 026 runtime asset manager in `doc/roadmap.md`
- [X] T053 Move native Metal to Feature 027 and split DX12, desktop OpenGL, and GLES into Features 031 through 033 in `doc/roadmap.md`
- [X] T054 Split Meshlet work into Features 028 and 029 and move Streaming & Residency to Feature 030 in `doc/roadmap.md`
- [X] T055 Split Ray Tracing into Features 034 and 035 and Global Illumination into Features 036 through 038 in `doc/roadmap.md`
- [X] T056 Update overview, DAG, tracks, solo order, risks, prompts, and changelog for Roadmap 2.1 in `doc/roadmap.md`
- [X] T057 Synchronize Roadmap 2.1 requirements, research decisions, schema, quickstart, plan, and range validation across `specs/002-engine-development-roadmap/`
- [X] T058 Correct project-level Speckit context and conventional auto-commit messages in `AGENTS.md` and `.specify/extensions/git/git-config.yml`
- [X] T059 Validate unique Features 003 through 038, backward-only dependencies, complete prompts/details, next-feature dry-run 020, and removal of active Roadmap 2.0 ranges

---

## Phase 9: Roadmap 2.2 Production-Content Gate Amendment

**Purpose**: Record the completed insertion of Production Content Integration
& Acceptance after Metal and before advanced future phases.

- [X] T060 Insert Feature 028 Production Content Integration & Acceptance after Feature 027 and preserve completed Features 003-027 in `doc/roadmap.md`
- [X] T061 Shift the then-future Features 028-038 to 029-039 and update the roadmap overview, DAG, detail sections, prompts, tracks, solo order, risks, and change log
- [X] T062 Record the Feature 028 source-to-cooked-to-visible acceptance boundary, tiered evidence, and explicit asset-license/compliance exclusion
- [X] T063 Mark Feature 028 complete using the final hosted and M4 Metal evidence while preserving the one-time Windows carry-forward provenance limitation
...
- [X] T064 Preserve completed Features 003-028 and insert Feature 029 HDR Post-Processing & Output Transform plus Feature 030 Anti-Aliasing & Temporal Reconstruction in `doc/roadmap.md`
- [X] T065 Renumber only the former future Features 029-039 to 031-041 across TOC, overview, DAG, detail headings, prompts, tracks, solo order, risks, and current-state text in `doc/roadmap.md`
- [X] T066 Keep Feature 031 Meshlet dependencies exactly 024, 025, 026, and 028; update downstream dependencies by semantic phase identity rather than by mechanical number alone in `doc/roadmap.md`
- [X] T067 Specify Feature 029's backend-neutral HDR SceneColor path, insertion points, manual exposure, versioned tone mapping, output transfer, Forward/Deferred unification, Render Graph, Vulkan/Metal presentation/readback, resize, debug bypass, and explicit exclusions in `doc/roadmap.md`
- [X] T068 Specify Feature 030's TAA-before-tonemap main path, FXAA-after-tonemap fallback, deterministic jitter, previous/current ViewProjection, motion vectors, history/reprojection/rejection/clamp, invalidation, static convergence, dynamic motion, Deferred `sampleCount=1` default, and explicit MSAA/vendor-upscaler exclusions in `doc/roadmap.md`
- [X] T069 Require Feature 039 Screen-Space GI to reuse Feature 030 motion-vector, jitter, history, reprojection, rejection, and invalidation contracts without a duplicate temporal framework in `doc/roadmap.md` and `specs/002-engine-development-roadmap/`
- [X] T070 Preserve Feature 028 v2 `sampleCount=1`/no-general-post-processing references as historical evidence; require revisioned exact-dimension SDR Candidates, explicit maintainer acceptance, no alignment/crop/scale/resampling, and bounded PNG/JSON evidence; and record the Feature 029 clarification that HDR visual authority uses a bounded macOS live-view maintainer attestation rather than automated comparison
- [X] T071 Synchronize current requirements, research decisions, plan, data model, quickstart, schema, checklist, and task coverage across `specs/002-engine-development-roadmap/`
