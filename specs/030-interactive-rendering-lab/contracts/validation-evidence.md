...
- macOS Metal native: UI, output/mode lifecycle and HDR nonvisual numeric contracts. No hosted HDR appearance claim. For Feature 030, macOS validation covers Apple Silicon arm64. By the maintainer decision on 2026-09-08, skip Intel x86_64 macOS build, cooking/derivation and native validation; record these cases as explicitly skipped, never passed. Existing x86_64 implementation/profile support and historical evidence remain unchanged. Native hardware coverage identifies the actual architecture, never inferred universal support.
...
| Mode-transition stress | Sponza on Windows; Lantern on M4 | 20 Windows cycles of capability-constrained SDR requests plus a capability-constrained HDR probe; 20 M4 cycles SDR -> PQ1000 -> PQ2000 -> EDR1000 -> EDR2000 -> SDR |
...
## 2026-09-16 capability correction

Coverage-v2 and Report-v2 supersede v1 for the new software freeze. Windows profile requests use the exact native format/color-space inventory, not a presumed universal set of SDR choices. Distinguish switched, retained-current and rejected-unsupported outcomes. Rejection must preserve complete settings state and output generation; every completed request links a subsequent queued presentation to its effective profile/settings/output identities. Capability query errors or changes, asynchronous failures and timeouts are failures. The independent consumer reconstructs support from the frozen profile registry and the native inventory linked to the report capability digest, and checks ordered script/result correspondence. No unsupported required default profile or required M4 HDR case can be skipped into green. The fixed case counts, cycles, budgets and formal/human gates are unchanged.

### Accepted reference identity across fresh captures

Formal SDR closeout binds each fresh Candidate to its own report PNG, current-revision calibration, and same-frame native readback. An existing Accepted reference retains its historical repository-relative path. The consumer independently bounds and reads that PNG, checks its compressed SHA-256, decoded RGB SHA-256 and exact 512x512 extent against the Accepted record, and checks the fresh Candidate against the same record. Filesystem path equality between these two independently verified images is not required. No tolerance, resampling, acceptance promotion, or stale capture substitution is introduced.

The 74e1c435 path-binding failures and successful machine/human evidence remain historical. This validation-policy correction requires a new exact software freeze, thirteen hosted jobs, five Windows and fourteen M4 machine cases, fresh formal captures and current human requests. Old artifacts may reproduce the regression but cannot count as new-freeze acceptance. Intel macOS remains explicitly skipped.
