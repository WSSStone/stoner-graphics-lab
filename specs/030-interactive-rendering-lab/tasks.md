...
**T015 progress**: The Metal production ownership, timeout/failure and deferred-override fixtures are implemented. The private Vulkan presentation policy now has 70 independently passing deterministic assertions in `Tests/VulkanLabPresentationPolicyTests.cpp`, registered with rhi-deferred-submission. They cover optional selection and enablement retry, retained query diagnostics, image-indexed reacquisition, canceled-image retention, predecessor caps and terminal assurance. Required native deferred submission and borrowed-swapchain Debug/Release probes now pass. Remaining delayed/failure boundary coverage and the process watchdog stay open; the policy tests alone do not close T015.
