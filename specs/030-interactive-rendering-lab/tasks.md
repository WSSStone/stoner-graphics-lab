...
**T015 progress**: The Metal production ownership, timeout/failure and deferred-override fixtures are implemented. The private Vulkan presentation policy now has 54 independently passing deterministic assertions in `Tests/VulkanLabPresentationPolicyTests.cpp`, registered with rhi-deferred-submission. They cover optional selection and enablement retry, retained query diagnostics, image-indexed reacquisition, canceled-image retention, predecessor caps and terminal assurance. Native deferred submission, real swapchain integration and the process watchdog still require validation; the policy tests alone do not close T015 or T021.
