...
**T032 review**: The UI-off lab now loads the existing strict-cooked scene/output closure, adapts the calibration camera to the actual drawable, and runs two deferred preview slots through the Renderer ticket and borrowed native target seams. Equivalent native BGRA8 SDR storage is explicitly bound only for zero-readback preview; formal output identity stays canonical. Actual Metal and forced-acquire-history Vulkan resize/minimize/restore tests pass. Debug/Release each passed 391 related assertions, 67 additional native regressions, and two 33-assertion scene lifecycle invocations; five Release Lantern/Sponza backend/mode smokes passed. Final fallback cleanup records IdleAssumed, independently of render completion. T033 operation counters, T034 checkpoint evidence, and T015 remaining failure coverage are still open. UI-on remains explicitly unavailable until US2 is implemented. See Validation/030/README.md.
