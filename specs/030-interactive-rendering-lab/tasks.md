...
**T064 render-resource increment**: `FLabProductionFrameContext::ReconfigureOutputSettings` validates a complete output policy and waits for every slot to become render-retired/idle. It then releases render-only bundles even at unchanged extent while preserving independent presentation leases and borrowed targets. New borrowed output formats rebuild lazily through the existing slot/aggregate budgets. Deterministic fixtures cover pending render rejection, invalid-policy preservation, same-extent SDR-to-float-HDR rebuilding while old presentation remains unsignaled, and separate final presentation retirement. Strict Debug/Release each pass 157 production/output assertions; architecture verification has zero findings. Logs: `Build/Validation/030/us2/output-recreation-{debug,release}.log`. This does not yet perform native swapchain mode switching; T064 remains open for Demo transition orchestration, actual former-output usability and native failure/fallback handling. Reviewed count remains 61/127.

**T064 output-usability increment**: Native facade status now requires a valid Ready/Acquired swapchain whose resolved image generation matches the active native generation; prior preparation alone cannot authorize resuming a retired output. The native suite verifies unprepared/paused/resumed status and generation identity. It also exposed a close-between-render-and-present race: Metal now retains an unsubmitted borrowed drawable with NotReady during close/minimize/zero extent for session cancellation or resume. A direct real-render-then-close regression proves cancellation without an admitted presentation lease. Strict Debug/Release builds pass; each configuration passes 45 native assertions with both Metal and Vulkan scene selections, including three resize/minimize cycles, zero image readbacks/live idles and zero final presentation owners. Architecture verification has zero findings. Logs: `Build/Validation/030/us2/output-usability-*`. Replacement-creation failure injection and full mode-switch orchestration remain pending; T064 stays open and reviewed count stays 61/127. These are local implementation checks, not formal/HDR authority.

**T063/T064 native output-switch increment**: Demo resolves a complete backend-correct profile inventory from advertised format/color-space pairs and EDR support, using the existing 029 resolver and its Metal-only HDR policy. Startup registers profile controls through the feature-owned section seam. A retained immutable transaction now drives render drain, native presentation replacement, render-bundle rebuilding and coherent settings commit while the event loop remains live. Acquire-history predecessors keep current-output acquisitions/presentation progressing before another generation is admitted; stale display transactions relinquish their token before lifecycle resize. Native replacement success updates the actual owner bindings before reporting settings completion. Runtime diagnostics preserve requested intent separately from effective output. Strict Debug/Release each pass 339 related CPU assertions and 47 assertions in each Metal/Vulkan/forced-fallback native selection. With UI enabled, M4 Metal completes seven actual profile changes (both PQ targets, both EDR targets, three SDR profiles), presenting two frames per committed mode and composing UI on all 16 frames. MoltenVK currently advertises neither BT.709 nor Gamma 2.2 color spaces, so their requests reject without replacing sRGB; these runs do not claim alternate Vulkan output-mode coverage. All paths retain zero image readbacks/live idles and zero final native presentation owners; fallback terminal cleanup remains explicitly IdleAssumed. Architecture verification has zero findings. Logs: `Build/Validation/030/us2/output-switch-*`. T063/T064 remain open for remaining controls/debug work, replacement-failure injection and capability-loss/transition recovery. Reviewed count remains 61/127; no formal or human HDR authority is claimed.
