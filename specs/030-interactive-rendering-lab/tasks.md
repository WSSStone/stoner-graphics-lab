...
**T034 progress**: Both suites are registered and pass locally in Debug/Release. Preliminary UI-off Lantern/Sponza smokes now exist, but the integrated lifecycle checkpoint and `us1.json` remain open. Repeated resize/minimize transitions can fail on Metal or time out on Vulkan acquire history; resolve these failures before accepting US1. Backend clear-frame probes and successful short scene runs do not close this checkpoint.
