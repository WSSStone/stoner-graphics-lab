...
`native-command.json` contains a `nativeCommand` argv array, with each argument a separate string and no shell expansion, plus expected coverage case ID, gate kind (smoke/endurance/lifecycle) and workload/backend/profile identity from Coverage-v1.json. The runner performs before/after software guards, invokes bounded cases and rejects stale output paths. Its bounded schema, argument parser and independent consumer are implemented; use [hardware-validation.md](hardware-validation.md) for exact command files and case recipes. Run/verify/closeout share one thin entrypoint and reuse existing provenance/artifact/image helpers; independent consumption still runs in a fresh process/job.
...
Expand **Capture**, enter a name using ASCII letters, digits, underscores or hyphens, and click **Capture output**. **Include UI** defaults off. SDR writes PNG and JSON into the configured export directory; HDR writes a numeric report there and raw bytes under `Build/InteractiveLab/Raw`, never an HDR appearance PNG. To export a selected diagnostic stage, first select its numeric mode, then click **Capture selected numeric stage**. At most two requests are retained; additional requests report Busy. Existing output files are not replaced. Watch the panel status for completion or a controlled rejection. Captures use actual drawable pixels (which can differ from logical window size on Retina), without resizing. These exports never update Accepted references. Encoding and publication run in one background task; the panel remains serviceable while they are pending. The request retains its capacity until that task retires. Completed GPU mapping stays explicit on the owner thread; exports do not add GPU waits.

## Implemented script/report entrypoint (T102)

`--lab-input-script PATH` requires `--mode validate --interactive-lab --frames N`. `--lab-report PATH` is also available in ordinary interactive lab sessions. Its parent must already exist under Build; publication rejects an existing filename and uses exit code 7 on report failure. These bounded reports are separate from formal acceptance.

The versioned Settings-v1 and Lifecycle-v1 files under `Config/Validation/InteractiveLab/Scripts/` contain at most 256 ordered steps within 64 KiB. Each step has `afterPresented`, `action`, and `value`; resize/restore also require `value2` for height. Actions are exposure, white, tone, profile, rejectProfile, debug (0 off/1 visualization/2 numeric selection), ui, focus, minimize, restore, resize, scale and close. Scale is a validation-only persistent content-scale override. The script waits for accepted settings to become effective and allows three new present-queued frames between ordinary steps; five seconds without script or presentation progress fails the run. Recovery events can proceed during a pending transition. Focus/minimize/restore actions inject typed events and claim no physical desktop action. Numeric debug selection alone never requests a capture.

For a prepared strict-cooked command, add `--mode validate --frames 120 --lab-input-script Config/Validation/InteractiveLab/Scripts/Settings-v1.json --lab-report Build/Validation/030/settings-new.json`. Repeat with Lifecycle-v1.json and a new report filename. Local checks exercised both scripts on Lantern/Sponza Metal and Vulkan; these do not replace frozen final hardware or human gates.
