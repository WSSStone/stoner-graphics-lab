...
- [ ] T122 Require the physical M4 Pro self-hosted arm64 job to pass offscreen conformance, shader cook, triangle/deferred comparison, failure, and lifecycle gates; cross-check its artifact with the local visible run and record accepted evidence in Validation/027/reports/native-arm64.json
- [ ] T123 Require the GitHub-hosted `macos-26-intel` x86_64 job to pass the equivalent full native Metal gates with a real `MTLDevice`, then record architecture/OS/device/capability plus GPU-readback evidence in Validation/027/reports/native-x86_64.json; physical Intel Mac validation is optional
