...
- [X] T135 [US3] Release unused Linux/glibc heap pages after complete native teardown at the exact warm-up and terminal comparison cycles before the unchanged `/proc/self/statm` samples, preserve all intermediate/peak evidence plus exact 20/2 and 1000/20 comparisons and the 16 MiB threshold, and rerun the hosted Lavapipe regular gate while retaining medium failure evidence
- **Historical, superseded T136**: [US3] Preserve concurrent disjoint medium CPU/cook stages under the unchanged shared 1,800-second deadline, record the isolated Lavapipe infeasibility, concurrent Metal GPU/RSS contention, and lock-only native/CPU overlap from run 32858413208, move the weekly/manual medium lane to hosted arm64 Metal, barrier after both strict-runtime stages, serialize its native lifecycle stages without weakening 1,000/20 or RSS contracts, and rerun the complete medium gate
- [X] T137 [US3] Preserve synchronized Deferred FinalOutput and ForwardColor GPU readback, nonblank validation, and exact two-capture accounting on every lifecycle cycle while limiting the five intermediate Deferred attachment copies plus retained evidence bytes and SHA-256 work to one bounded authoritative extraction; prove the selector, seven retained readbacks, 40 regular captures, and unchanged lifecycle/RSS contract in Demo/StonerDemo/Private/FProductionContentDeferredExecution.*, Demo/StonerDemo/Private/FStonerDemoProductionContentRun.cpp, and Tests/ProductionContentDemoTests.cpp
- **Historical, superseded T138**: [US3] Converge Linux/glibc comparison-point heap release with a bounded `malloc_trim(0)` loop that stops when no further pages are released, retain the exact warm-up/terminal `/proc/self/statm` samples and 16 MiB limit, and rerun hosted Linux Vulkan regular evidence after run 32858413208 exposed single-pass allocator timing
- **Historical, superseded T139**: [US3] Keep 20 per-target clean determinism repetitions authoritative in the regular gate while medium/hardware execute one clean plus one 100-percent-reuse warm cook per package, report the actual clean-run count, retain every 1,000/20 native/RSS/readback/ownership requirement, and rerun hosted medium after barrier revision 3e52037 passed locally in 1,756.410 seconds but run 32864192139 exhausted the hosted budget
- **Historical, superseded T140**: [US3] Preserve the passing bcd07b6 full-profile M4 Pro evidence and replace the scheduled/manual single-host medium lane with one isolated macOS arm64 Metal lane per exact profile package, retain the unchanged 1,800-second budget and complete clean/warm/strict/equivalence/1,000-cycle/RSS contract on every lane, add a fail-closed exact-package/corpus/target/revision aggregate, and rerun hosted medium after run 32869040060 exhausted the single-lane budget
- **Historical, superseded T141**: [US3] Converge released macOS allocator-zone pages with bounded `malloc_zone_pressure_relief(nullptr, 0)` calls at the unchanged warm-up and terminal comparison points, retain the exact 1,000/20 lifecycle, ownership, stale-handle, process-RSS, and 16 MiB contracts, and rerun the isolated hosted Metal lanes after run 32874425910 completed Lantern ownership proof but reported 26,411,008 bytes of RSS growth
- **Historical, superseded T142**: [US3] Move the seven full-resolution authoritative raw readbacks to one uncounted post-lifecycle extraction on the same strict generation/backend, retain exact 1,000/20 cycles, 2,000 captures, terminal teardown/RSS, seven readbacks, semantic/FLIP, and a second owner/stale check, and rerun hosted Metal after run 32877744103 showed that pre-sample evidence retention inflated Lantern RSS growth to 68,403,200 bytes
...
- [X] T144 [US3] Eliminate redundant strict-runtime payload hashing only when a validated generation manifest supplies the exact cooked-envelope digest, retain the generic body-digest path and fail-closed mutation behavior, load only the Sponza v2 1,000-cycle root closure before cache-backed manifest completeness checks while preserving the Lantern and regular schedules, preserve eight workers and every 1,000/20 lifecycle/image/ownership/RSS requirement, and prove Sponza locally within the unchanged 1,800-second lane budget
- [X] T145 [US3] Add an explicitly enabled, manifest-bounded Sponza v2 1,000-cycle authentication memo that owns a continuous reader lease for the exact publication namespace and generation, records only successfully authenticated envelope digests, never retains payload/runtime/graphics objects, keeps complete file reads, typed decode, realization, rendering, and teardown on every cycle, rejects first corruption and mismatched bindings, leaves Lantern/regular/default manager paths unchanged, and pass the canonical local medium runner in 1,094.29 seconds with 1,000 cycles, 2,000 captures, seven readbacks, zero owners, stale rejection, and 2,850,816 bytes of RSS growth
- [X] T146 [US3] After hosted run 32907674313 exposed 25,837,568 bytes of Lantern RSS growth and another Sponza native timeout, decode previously authenticated envelopes from a call-bounded borrowed body span instead of allocating a second full body copy, retain complete file reads and all header/schema/typed validation, preserve owned-body public/generic paths, extend the bounded memo only to accepted Lantern/Sponza v2 1,000-cycle profiles, reject 20-cycle/unknown profiles, and prove local Lantern full-medium plus Sponza native-only 1,000-cycle gates with seven readbacks, zero owners, stale rejection, and at most 2,031,616 bytes of RSS growth
- [X] T147 [US3] After hosted run 32923918933 completed Lantern's 1,000 cycles but still reported 59,506,688 bytes of RSS growth, evaluate a one-worker Lantern v2 1,000-cycle allocator hypothesis without changing lifecycle work, pass locally in 668.94 seconds with 770,048 bytes of growth, reject the hypothesis when hosted run 32927821133 instead reported 92,979,200 bytes of growth, and restore Lantern to eight workers while retaining the experiment as evidence
- [X] T148 [US3] After the same hosted run gave Sponza 1,684 seconds of native execution and still timed out, increase only the accepted Sponza v2 1,000-cycle manager selection from eight to sixteen workers within the validated 32-worker bound, keep Lantern and regular worker policies unchanged, retain complete per-cycle loading/typed decode/realization/render/release plus every lifecycle/RSS/readback/ownership contract, and pass a direct local 1,000-cycle replay with seven readbacks, zero owners, stale rejection, and 1,966,080 bytes of RSS growth
- [X] T149 [US3] After hosted run 32927821133 showed both a 92,979,200-byte Lantern terminal increase and a 50,479,104-byte Linux regular endpoint difference despite zero owners, add an unconditional value-independent comparison-point protocol of bounded heap relief, fixed 100-millisecond native/allocator quiescence, and bounded relief before the unchanged single warm-up/terminal RSS samples, preserve exact cycles and the 16 MiB threshold, restore Lantern to eight workers, and pass strict Release, 42 Core platform tests, 27 strict-runtime tests, and a 20-cycle native Metal replay with seven readbacks, zero owners, stale rejection, and 5,406,720 bytes of growth
- [X] T150 [US3] After the sixteen-worker Sponza native stage in hosted run 32927821133 still exhausted its exact 1,631-second allowance, replace the validation session's per-cycle deep copies of immutable typed payloads with aliasing shared pointers that retain the same `TAssetHandle` control and authoritative external-retention accounting, keep complete strict reads/typed decode/realization/render/teardown and fail-closed lifetime behavior, prove alias lifetime semantics plus strict Release, and pass the exact local Sponza and Lantern 1,000-cycle replays in 947.82 and 176.82 seconds respectively with 2,000 captures, seven readbacks, zero owners, stale rejection, and at most 2,031,616 bytes of RSS growth
- [X] T151 [US3] After hosted run 32932377467 still timed out Sponza after 1,687 native seconds, correct the composition lifetime so one generation-bound, payload-free authentication context and reader lease survive all manager cycles instead of being destroyed with `FProductionContentRuntime`, keep complete per-cycle manager/handle/decode/realization/render/native teardown, add a 1,000-cycle integration assertion for actual cross-cycle reuse, and pass exact local Sponza/Lantern replays in 780.84/169.68 seconds with seven readbacks, zero owners, stale rejection, and at most 884,736 bytes of RSS growth
- [X] T152 [US3] After the 100-millisecond fixed quiescence passed one hosted Linux regular run with zero positive growth but run 32932377467 again reported 50,049,024 bytes between its exact cycle-2 and cycle-20 samples despite zero owners, retain Metal's 100-millisecond protocol, use a fixed one-second value-independent glibc/Lavapipe quiescence between the same bounded trim sequences, keep exactly one sample at each unchanged comparison point and the 16 MiB threshold, and pass strict Release plus all 42 Core platform tests before hosted Linux revalidation
- [X] T153 [US3] After hosted run 32936488807 passed every regular, sanitizer, artifact, and Lantern medium gate but Sponza again exhausted its exact 1,668-second native allowance, retain the first successful generation pointer/manifest/layout result as bounded payload-free authority under the existing continuous reader lease, validate every new Manager against the exact namespace/generation/target/extensions/record authority without repeating filesystem enumeration, combine strict bounded regular-file validation and complete reading through one native handle, preserve all 189 complete file reads and typed decode/realization/render/teardown work per cycle, and pass the exact local Sponza 1,000-cycle Metal replay in about 620 seconds with 2,000 captures, seven readbacks, zero owners, stale rejection, both reuse assertions, and 2,686,976 bytes of RSS growth
- [X] T154 [US3] After hosted run 32942777805 passed every regular, sanitizer, artifact, and Lantern medium job but Sponza still exhausted its exact 1,672-second native allowance, replace its single-worker recursive root-first closure load with deterministic dependency-first batches, let the same Manager borrow only already-loaded immutable dependencies between batches, retain one complete read/parse/typed decode for every one of the 189 manifest records on every lifecycle cycle plus exact 1,000/20/render/readback/ownership/RSS contracts, pass the exact local Sponza Metal replay in about 390 seconds with 2,000 captures, seven readbacks, zero owners, stale rejection, both reuse assertions, and 98,304 bytes of RSS growth, then prove hosted run 32951285909 completes the full Sponza 1,000-cycle native workload inside its lane deadline before exposing the successor RSS-boundary failure
- **Historical, superseded T155**: [US3] After hosted run 32951285909 completed Linux Vulkan regular and Sponza Metal medium with every tracked owner at zero and stale-handle rejection but reported 108,912,640 and 92,946,432 bytes of endpoint RSS growth, fully release the submission harness and requested native backend before the exact Linux Vulkan 20/2 and macOS Metal 1,000/20 headless comparison samples, restore and re-prove the same backend after warm-up and before post-lifecycle readbacks, keep visible hardware continuous, retain one unconditional sample, fixed quiescence, unchanged cycles and 16 MiB limit, pass strict builds/regressions plus a local Sponza replay with exactly two recycles and 2,424,832 bytes of growth, then rerun hosted Linux and Metal authority
- **Historical, superseded T156**: [US3] After hosted run 32956455256 proved both exact Linux backend recycles executed but still measured 32,063,488 bytes between a cycle-2 sample after the initial Vulkan device and a terminal sample after its replacement, and run 32960932852 passed Linux with an unmeasured restart prime but exposed the same Metal asymmetry at 94,126,080 bytes after all 1,000 cycles, follow the existing unmeasured Deferred/Forward prime with one unmeasured full backend shutdown, platform-specific fixed relief, and native-proved restart before cycle 1 for both exact native-headless RSS authorities; require exactly one prime plus both comparison recycles, retain exact 20/2 and 1,000/20 cycles, one value-independent sample per endpoint, zero-owner/stale checks and 16 MiB limit, pass strict builds/architecture plus the exact local Sponza replay with 3,604,480 bytes of growth and exactly three recycles, then rerun hosted Linux and Metal authority
- **Historical, superseded T157**: [US3] After hosted run 32956455256 exhausted the 1,676 native seconds remaining inside Sponza's 1,800-second complete-profile deadline, while run 32951285909 required about 1,631 native seconds before the mandatory full-backend RSS boundary work and about 139 seconds of preceding stages, revise the isolated medium package-lane contract to 2,100 seconds across schema, runner, aggregate, workflow, tests, quickstart, and specifications; retain the 60-minute job bound plus every 1,000/20 lifecycle, complete read/decode/render, seven-readback, image, ownership, stale-handle, exact RSS, and 16 MiB requirement, then rerun both medium shards and exact aggregate authority
- **Historical, superseded T158**: [US3] After full hosted run 32965832757 passed Windows/Linux/arm64 regular, Lantern medium, ASan/UBSan, and TSan but Sponza retained 93,061,120 bytes of endpoint RSS after three exact backend recycles and x86_64 Metal consumed its 600-second profile deadline across all 20 clean cooks, increase only the fixed Metal comparison-point quiescence from 100 milliseconds to one second between the same all-zone relief passes and execute at most four disjoint x86_64 Metal clean cooks concurrently; retain one value-independent sample per endpoint, 16 MiB, exact 1,000/20 and 20 clean byte-identical contracts, arm64/Vulkan scheduling, pass strict builds/runner/Core tests plus a local Sponza replay, then rerun complete hosted authority
- **Historical, superseded T159**: [US3] After hosted run 32976142026 proved the four-way x86_64 Metal clean schedule passes all 20 clean cooks inside the unchanged 600-second profile but Linux still reported 52,314,112 bytes of RSS growth after three recycles and Sponza exhausted the 1,938 native seconds remaining inside its 2,100-second profile budget, start only the exact Linux Vulkan 20/2 native-headless child with one glibc arena and revise isolated medium package authority to 2,400 seconds across schema, runner, aggregate, workflow, tests, quickstart, and specifications; keep fixed trim/quiescence, one sample per endpoint, 16 MiB, 60-minute outer job, exact 20/2 and 1,000/20 work, seven readbacks, owner/stale proof, pass local contracts, then rerun complete hosted authority
- **Historical, superseded T160**: [US3] After hosted run 32981721345 accepted the exact Linux single-arena authority at 2,371,584 bytes of RSS growth but reproduced Sponza's approximately 93 MiB terminal increase after 1,000 cycles, three complete Metal backend recycles, all-zone relief, one-second quiescence, zero tracked owners, and stale-handle rejection, start only the exact macOS Metal 1,000/20 native-headless child with Apple libmalloc space-efficient mode; retain visible hardware/default allocator behavior, exact work, one fixed sample per endpoint, 16 MiB, seven readbacks, ownership/stale proof, and rerun complete hosted authority
- **Historical, superseded T161**: [US3] After full hosted run 33041706012 showed that space-efficient mode alone still leaves 95,240,192 bytes of Lantern endpoint RSS growth on macOS 26 arm64 after all 1,000 cycles, 2,000 captures, three backend recycles, zero terminal owners, and stale-handle rejection, additionally disable Nano and limit the general tiny/small allocator to one magazine only for the exact macOS Metal 1,000/20 native-headless child; retain default allocator behavior everywhere else, unchanged lifecycle/readback/ownership/stale/RSS/16 MiB contracts, pass runner tests and an exact local Lantern replay, then rerun complete hosted authority
- **Historical, superseded T162**: [US3] After full hosted run 33043257554 proved the exact arm64 Metal 1,000-cycle Lantern work still leaves 95,174,656 bytes of endpoint RSS because macOS 26 arm64 defaults to Xzone and Apple exposes only a development-only Xzone environment override that hosted production jobs cannot use, move only the scheduled/manual medium shards and their fail-closed aggregate to GitHub-hosted Intel Metal and `Mac-Metal-X86_64.json`, where Xzone is never enabled; retain arm64 regular coverage and physical M4 visible hardware authority, the exact 1,000/20 lifecycle, 2,000 captures, seven readbacks, ownership/stale/image proof, one fixed endpoint sample, 16 MiB limit, 2,400-second lane budget, and rerun complete hosted authority
- **Historical, superseded T163**: [US3] After Intel hosted run 33046135110 reduced Lantern endpoint RSS growth from about 95 MiB to 44,896,256 bytes but still failed after all 1,000 cycles, 2,000 captures, zero owners, and stale rejection, eliminate the validation path's redundant per-capture native and CPU allocation churn: copy Shared Metal buffers directly, explicitly synchronize Managed buffers before direct copy, retain staging only for Private buffers, and reuse one bounded native-headless CPU scratch vector; preserve every GPU submission, full-resolution readback, nonblank check, capture, seven-attachment extraction, lifecycle, image, ownership, stale, and 16 MiB requirement, pass repeated Shared/Managed/Private readback tests plus strict Debug/Release, prove exact local Lantern medium with 524,288 bytes of growth, then rerun complete hosted authority
- **Historical, superseded T164**: [US3] After Intel hosted run 33052483658 falsified T163 as a complete RSS fix by retaining 43,683,840 bytes after all 1,000 Lantern cycles, while Sponza completed every pre-native stage but received only the 2,112 seconds left in the shared 2,400-second deadline, add non-authoritative macOS task-VM and all-zone malloc endpoint diagnostics without changing the resident-size sample or 16 MiB gate; revise medium authority to a 3,000-second complete package lane with an independently capped 2,400-second native stage under the existing 60-minute workflow job; retain exact 1,000/20 work, seven readbacks, ownership/stale proof, pass strict builds and runner/workflow contracts, then rerun the isolated Intel medium shards and use the diagnostics—not another ungrounded allocator toggle—to determine any remaining Lantern fix
- **Historical, superseded T165**: [US3] After hosted run 33062479846 proved Lantern's 45,182,976-byte resident increase is explained almost entirely by 44,670,976 bytes of new TASK_VM reusable pages while physical footprint grew only 512,000 bytes and all-zone malloc grew to 37,662,544 bytes in use / 68,362,240 bytes allocated, disable the scalable allocator's 128 MiB medium region only for the exact Intel Metal 1,000/20 native-headless child while retaining the existing space-efficient, Nano, magazine, all-zone relief, one-sample resident-size, and 16 MiB contracts; after the same run cancelled Sponza 2,053 seconds into native work at the enclosing 60-minute job deadline, bound the workflow job to 90 minutes without changing the 3,000-second complete-lane or 2,400-second native caps; pass runner/workflow contracts plus strict builds and local exact Lantern, then rerun both Intel shards and aggregate authority

**Historical investigation ledger**: Entries explicitly prefixed `Historical,
superseded` are evidence records, not executable tasks and not incomplete work.
...
- [X] T181 [P] [US3] Add failing same-frame, real stale-token, measured probe-ID, multi-reference schema/selection, report-v3, cross-process calibration, package collect-all, and split-budget contract tests.
- [X] T182 [US3] Bind authoritative Deferred attachments, FinalOutput, and presented capture to one completed submission frame token; reject mixed/stale bundles and remove lifecycle-counter self-attestation.
- [X] T183 [US3] Emit bounded privacy-safe CPU/render-state and per-attachment SHA-256 diagnostics sufficient to localize the first divergent stage without retaining raw images.
- [X] T184 [US3] Upgrade image baseline records to schema v2 reference sets with one to three ordered references, per-reference policy/calibration evidence, deterministic any-match selection, and complete comparison reporting.
- [X] T185 [US5] Upgrade the acceptance report to v3 with frame token, probe IDs, reference comparisons, matched reference, and bounded cross-process calibration summary.
- [X] T186 [US4] Split profile/package/native operational budgets, give hardware packages independent 3,600-second deadlines inside 7,800 seconds, and collect recoverable serialized package failures while retaining fatal integrity stops.
...
- [X] T189 Run final Windows Vulkan authority evidence without accepting a new baseline automatically; retain only small JSON and one lossless PNG per unique candidate mode, then complete explicit maintainer review and closeout evidence. Revision 0cf0182 produced same-frame semantic evidence and the stable Sponza mode; the maintainer explicitly accepted it on 2026-08-30 and admission retains only the canonical PNG, registry record, and bounded calibration/index evidence.
...
- [X] T192 [P] [US4] Align the medium aggregate validator and its direct checked-in-profile regression with validation-profile schema v3 after run 33318787989 proved both medium shards but rejected the stale v2 aggregate contract.
- **Historical, superseded T193**: [US4] After run 33326293909 reached cycle 900 before exhausting the 4,800-second hosted Intel native watchdog, preserve exact 1,000/20 lifecycle, 2,000 captures, seven readbacks, owner/stale and report gates while expanding only hosted medium package/profile/native operational envelopes to 6,600/6,900/6,000 seconds inside a 150-minute job; T194 later replaced this duplicated endurance contract while retaining physical authority.
- [X] T194 [US4] Supersede the repeated hosted Sponza timeout expansion with validation-profile schema v4 and ordered per-package lifecycle purposes: retain Lantern `endurance` at 1,000/20 and 2,000 captures, define Sponza `scale-lifecycle` at 100/10 and 200 captures with complete strict closure/read/decode/realize/render/release work on every cycle, retain seven readbacks plus owner/stale hard gates for both, restore hosted medium operational bounds to 2,400/2,700/1,800 seconds inside a 90-minute job, keep both physical packages at 1,000/20 with unchanged image authority, and update runner/aggregate/workflow/C++ contract tests plus all Feature 028 design and operator documentation.
- [X] T195 [US5] After hosted run 33362940103 attempt 2 proved a successful failed-job rerun could not revalidate unchanged attempt-1 producer artifacts, preserve immutable attempt-suffixed evidence while resolving each regular consumer to the newest non-expired producer artifact at or before the current attempt within the same workflow run; fail closed on missing, future, expired, malformed, or ambiguous records, add resolver/workflow contracts, and do not weaken any producer or digest-revalidation gate.
- [X] T196 [US3] After the final local Metal authority run proved a 512-by-512 logical GLFW window became a 1024-by-1024 Retina drawable and correctly failed `presented-frame-pixels`, add an explicit high-density framebuffer preference plus bounded measured-drawable/client-extent convergence only for formal visible production acceptance, preserve ordinary/preview high-density behavior, cover descriptor propagation and 2x/equal/invalid extent calculation, and retain exact same-frame byte equality without scaling or alignment.
- [X] T197 [US3] Calibrate the corrected exact-drawable Metal Lantern output across three independent processes and 60 exact 512-by-512 captures, prove one zero-noise mode plus rejection of blank, stale, origin, one-pixel translation, missing-geometry, material-swap, color-space, and opposite-normal mutations, obtain explicit maintainer acceptance on 2026-09-01, and admit the losslessly compressed reference as `production-content-lantern-v2.macos.apple8.metal.rgba8.v2` without alignment or resampling.
- [X] T198 [US3] Calibrate the corrected exact-drawable Metal Sponza output across three independent processes and 60 exact 512-by-512 captures, prove one zero-noise mode plus rejection of blank, stale, origin, one-pixel translation, missing-geometry, material-swap, color-space, and opposite-normal mutations, complete the 1,000/20 physical lifecycle with 2,000 captures, seven readbacks, zero owners, stale rejection, and zero RSS growth, obtain explicit maintainer acceptance on 2026-09-01, and admit the losslessly compressed reference as `production-content-sponza-v2.macos.apple8.metal.rgba8.v2` without alignment or resampling.
...
- [X] T117 Update Feature 028 status, final evidence references, and project memory without rewriting historical gaps in doc/roadmap.md, specs/028-production-content-acceptance/spec.md, AGENTS.md, and Validation/028/README.md; the then-current Meshlet next-phase pointer is superseded by Roadmap 2.3, where Feature 029 is HDR Output, Feature 030 is AA/Temporal, and Meshlet is Feature 031
...
- T166, T167, and T171 cover image-region semantics, profile authority, and
  report authority in separate files before their integration tasks.
