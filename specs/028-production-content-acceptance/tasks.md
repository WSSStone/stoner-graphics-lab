...
- [ ] T156 [US3] After hosted run 32956455256 proved both exact Linux backend recycles executed but still measured 32,063,488 bytes between a cycle-2 sample after the initial Vulkan device and a terminal sample after its replacement, follow the existing unmeasured Deferred/Forward prime with one unmeasured full backend shutdown, fixed glibc/Lavapipe relief, and native-proved restart before cycle 1, require exactly one prime plus both comparison recycles, retain the exact 20/2 cycles, one value-independent sample per endpoint, zero-owner/stale checks and 16 MiB limit, pass strict builds and architecture regressions, then rerun hosted Linux authority
