...
### T100 — interactive capture export (2026-09-15)

**T100 review (2026-09-15)**: Capture UI actions now enqueue exact current-drawable requests, optionally include UI, and consume only completed/reset staging. SDR exports exact-size PNG plus bounded provenance JSON; HDR and selected stages export numeric raw data under ignored Build with a bounded report and no appearance claim. Protected no-replace publication is shared with presets. Vulkan enables optional CopySource only when both surface and format support it; absence rejects capture without blocking preview. Strict Debug/Release each pass 333 CPU assertions; Metal Debug passes 86 native assertions and Vulkan Release passes 69. Independent validation verifies nine exports, PNG CRC/decompression/dimensions, payload hashes and numeric aggregates. Local evidence: `Build/Validation/030/us6-capture-ui.json`. Overall 100/127 reviewed; T101 event-service timing, remaining US6 and formal/human closeout remain pending. Large explicit encoding/publication is currently synchronous and its event-service latency is not yet qualified. Intel macOS remains skipped.

### T101 — bounded export worker and lifecycle review

**T101 review (2026-09-15)**: PNG/numeric encoding and protected file publication now run in one CPU-only export worker. The event owner polls without waiting, retains the staging alias and queue charge until completion, and admits no second worker. Terminal cleanup polls the worker before device teardown under the existing independent five-second drain/ten-second watchdog; blocked publication cannot race native destruction. Completed GPU mapping remains explicit and bounded on the owner thread, with no GPU readback wait. Existing session handling retains <=16 ms busy/transition and <=50 ms minimized wait slices, zero first-resumed camera delta and first-error preservation. Strict Debug/Release each pass 338 focused assertions plus the watchdog subprocess fixture; Metal Debug/Vulkan Release pass 86/69 native assertions, with nine export payload hashes independently checked. The watchdog fixture uses a shortened injected deadline; full ten-second and unreleased-owner qualification remains T103. This closes T101 (101/127 reviewed), not US6/final hardware authority. Intel macOS remains skipped. Evidence: `Build/Validation/030/us6-export-worker.json`.

### US6 local implementation checkpoint

**T102–T104 review (2026-09-15)**: Strict bounded ordered scripts now drive the existing session/window seams through `--lab-input-script` in validate mode. Accepted settings must become effective before advancing; recovery events bypass pending-transition deferral and five seconds without progress fails. `--lab-report` writes bounded no-replace JSON under Build with actual script identity, native/readback and active/peak resource counters; reports remain non-authoritative and typed focus/minimize events are synthetic. Full ten-second and shortened watchdog subprocess fixtures preserve Forced/retained-owner failures. Strict Debug/Release each pass 320 focused assertions including script recovery and both watchdog fixtures; Metal Debug/Vulkan Release each pass 229 native UI/output/parity/capture assertions. Eight 120-frame Lantern/Sponza Metal/Vulkan Settings/Lifecycle CLI runs pass with zero implicit readbacks/live idles and final attachment/capture ownership zero. An impossible paused script fails after five seconds. Evidence: `Build/Validation/030/us6.json`. US6 local implementation checkpoint complete (104/127 reviewed). US7 hosted/physical/human authority remains pending; Intel macOS stays skipped.
