...
Stoner::Core::FString FVulkanPipelineCache::BuildGraphicsKey(const Stoner::RHI::FRHIGraphicsPipelineDesc& Desc) const
{
    ...
    Stream << "|blend:" << (Desc.Blend.bEnabled ? 1 : 0) << ',' << static_cast<int>(Desc.Blend.SourceColor) << ',' << static_cast<int>(Desc.Blend.DestinationColor) << ',' << static_cast<int>(Desc.Blend.ColorOp) << ",mask:" << static_cast<unsigned int>(Desc.Blend.ColorWriteMask);
